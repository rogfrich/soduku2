"""
This has been deprecated in facor of an OO approach (sudoku.py)
Once new tests are written for sudoku.py, this will be deleted.
Keeping it around until then so the existing tests don't break
"""
...
def get_impossibles(ref, test_grid=None):
    if (
        not test_grid
    ):  # if this func is called from the test harness, it will be supplied with an instance of the grid
        global grid
    else:
        grid = test_grid
    impossibles = set()
